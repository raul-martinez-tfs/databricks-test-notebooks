...
# MAGIC %sql
# MAGIC 
# MAGIC with cntrl_tower_ioh as (
# MAGIC select * from (SELECT 
# MAGIC concat(trim(finv.item_nbr),'|',trim(finv.plant_cd)) as ioh_sku_site_cd,
# MAGIC cast (fscl_yr_qtr_nbr as string ) as ioh_yearqtrnbr,
# MAGIC SUM(finv.on_hand_qty - finv.blocked_qty - finv.qa_inspn_qty - finv.rstrct_qty + finv.Consignment_qty) as ioh_qty,
# MAGIC 'gbl'  as src_sys_cd,
# MAGIC cast(current_timestamp as string) as rec_crt_ts,
# MAGIC cast(current_timestamp as string) as rec_updt_ts,
# MAGIC cast (fscl_yr_prd_nbr as string ) as ioh_yearmthnbr
# MAGIC 
# MAGIC FROM 
# MAGIC  f_invntry_bal_dly_hist finv
# MAGIC left join (SELECT distinct fscl_yr_prd_nbr , fscl_yr_qtr_nbr 
# MAGIC FROM d_date where fscl_yr_nbr between date_format(add_months(current_date,-12),'yyyy') and date_format(add_months(current_date,24),'yyyy')) QtrConv on cast(substring (finv.capture_dt,1,6) as string)=QtrConv.fscl_yr_prd_nbr 
# MAGIC WHERE finv.src_sys_cd IN ('gbl') and 
# MAGIC ( trim(finv.item_nbr) is not null or trim(finv.plant_cd) is not null) 
# MAGIC and finv.capture_dt in (select max(capture_dt) from  f_invntry_bal_dly_hist where src_sys_cd='gbl')
# MAGIC and trim(finv.plant_cd) in ('0010','1001','0070') and speclty_stk_type_cd = '?'
# MAGIC group by ioh_sku_site_cd, fscl_yr_qtr_nbr ,fscl_yr_prd_nbr
# MAGIC ) where ioh_yearqtrnbr is not null 
# MAGIC )
# MAGIC 
# MAGIC SELECT sum(ioh_qty), ioh_yearqtrnbr
# MAGIC FROM cntrl_tower_ioh
# MAGIC where ioh_yearqtrnbr='20231' and src_sys_cd='gbl'
# MAGIC group by ioh_yearqtrnbr, src_sys_cd
# MAGIC 
# MAGIC ;
...
# MAGIC %sql 
# MAGIC SELECT distinct on_hand_qty, blocked_qty, qa_inspn_qty, rstrct_qty, Consignment_qty
# MAGIC FROM f_invntry_bal_dly_hist
# MAGIC where capture_yr_mth_nbr in (202301, 202302) and src_sys_cd='gbl'
# MAGIC ;
...
# MAGIC     (on_hand_qty - blocked_qty - qa_inspn_qty - rstrct_qty + Consignment_qty) as ioh_qty
# MAGIC FROM f_invntry_bal_dly_hist
# MAGIC where capture_yr_mth_nbr in (202301, 202302) and src_sys_cd='gbl'
# MAGIC ;
...
# MAGIC select * from (
# MAGIC SELECT 
# MAGIC concat(trim(sup_dmd_table.item_nbr),'|',trim(sup_dmd_table.plant_cd) )  as fcf_sku_site_cd,
...
# MAGIC from
# MAGIC  f_forecast sup_dmd_table
# MAGIC left outer join (SELECT distinct fscl_yr_prd_nbr ,fscl_yr_qtr_nbr FROM  d_date where fscl_yr_nbr between date_format(add_months(current_date,-12),'yyyy') and date_format(add_months(current_date,24),'yyyy')) MthConv on sup_dmd_table.capture_yr_mth_nbr =MthConv.fscl_yr_prd_nbr
# MAGIC where sup_dmd_table.forecast_type ='FCF'  and  sup_dmd_table.src_sys_cd in ('gbl')
# MAGIC and  sup_dmd_table.capture_dt= (select max(sup_dmd_table.capture_dt) from  f_forecast sup_dmd_table where src_sys_cd = 'gbl' )
# MAGIC group by 
# MAGIC fcf_sku_site_cd,MthConv.fscl_yr_qtr_nbr,MthConv.fscl_yr_prd_nbr
# MAGIC ) where fcf_yearqtrnbr is not null 
# MAGIC  and src_sys_cd='gbl' and fcf_yearqtrnbr in ('20231')
# MAGIC     and fcf_qty is null
# MAGIC limit 10
# MAGIC ;
