...
# MAGIC 
# MAGIC from  F41021  
# MAGIC left outer join f4102_adt f4102 on trim(cast(cast(f4102.ibitm as integer) as string))= trim(cast(cast(F41021.LIITM as integer) as string))
# MAGIC and trim(F41021.LIMCU) = trim(f4102.ibmcu)
# MAGIC left outer join f4105
# MAGIC on  trim(cast(cast(F41021.LIITM as integer) as string)) = trim(cast(cast(f4105.COITM as integer) as string)) and trim(F41021.LIMCU) = trim(f4105.COMCU)
# MAGIC left outer join f0006 on f0006.mcmcu = f41021.limcu
# MAGIC 
# MAGIC left outer join f4101_adt f4101 on trim(f4101.IMITM) = trim(f41021.LIITM)
# MAGIC 
# MAGIC left outer join f0010 on f0010.CCCO= f0006.MCCO
# MAGIC 
# MAGIC left outer join d_date d_dt on date_format(if(date_format(current_timestamp,'%h')>='0' and date_format(current_timestamp,'%h')<='12',date_sub(current_date,1),current_date),'yMMdd') = cast (d_dt.dt_key as string)
# MAGIC 
...
# MAGIC from d_curncy_mth_rt curr_mnth where TO_CURNCY_CD =  'USD') co_curr_mth  on co_curr_mth.YR_MTH_NBR = d_dt.fscl_yr_prd_nbr and
# MAGIC co_curr_mth.FROM_CURNCY_CD = COALESCE(f0010.CCCRCD,'USD')
# MAGIC 
# MAGIC left outer join  edp_lkup hfm on hfm.lkup_key_01 = f0006.MCCO and  hfm.LKUP_TYP_NM ='CO_TO_HFM' and hfm.lkup_key_02 = 'E1LSG'
# MAGIC left outer join f0005 F0005_LOT on trim(F0005_LOT.DRSY)= '41' and trim(F0005_LOT.DRRT)='L' and trim(F0005_LOT.DRKY)=trim(f41021.LILOTS)
# MAGIC left outer join f0005 F0005_UOM on trim(F0005_UOM.DRSY)= '00' and trim(F0005_UOM.DRRT)='UM' and trim(F0005_UOM.DRKY)=trim(f4101.IMUOM1)
# MAGIC left outer join f0005 F0005_PROD on trim(F0005_PROD.DRSY)= '41' and trim(F0005_PROD.DRRT)='S3'  and trim(F0005_PROD.DRKY)=trim(f4102.IBSRP3)
# MAGIC 
# MAGIC left outer join f0005 f0005 on trim(f0005.DRSY)= '41' and trim(f0005.DRRT)='S2' and trim(f0005.DRKY)=trim(f4102.IBSRP2)
# MAGIC 
# MAGIC left outer join f0005 F0005_div on trim(F0005_div.DRSY)= '41' and trim(F0005_div.DRRT)='S1' and trim(F0005_div.DRKY)=trim(f4102.IBSRP1)
# MAGIC 
# MAGIC )
...
# MAGIC   left outer join ItemWOPD on (itmqtyOH.Item_No_=ItemWOPD.no_)
# MAGIC   left outer join location on (itmqtyOH.Location_Code=location.code)
